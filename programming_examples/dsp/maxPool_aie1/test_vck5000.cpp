...
constexpr int input_length = M*N*C;
constexpr int output_length = input_length / 4;
...
bool write_data_to_file(const std::string& filename, const uint8_t* data, size_t size) {
    FILE* file = fopen(filename.c_str(), "wb");  // Open file in binary mode
    if (!file) {
        std::cerr << "Failed to open file: " << filename << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < size; i++) {
        if (fwrite(&data[i], sizeof(uint8_t), 1, file) != 1) {  // Write the uint8_t value as binary
            std::cerr << "Error writing data to file: " << filename << std::endl;
            fclose(file);
            return false;
        }
        uint8_t newline = '\n';
        if (fwrite(&newline, sizeof(uint8_t), 1, file) != 1) {  // Write a newline character
            std::cerr << "Error writing newline to file: " << filename << std::endl;
            fclose(file);
            return false;
        }
    }
...
int main(int argc, char *argv[]) {
  ...
  uint8_t *in_a = (uint8_t *)mlir_aie_mem_alloc(xaie, buf0, input_length);
  uint8_t *out = (uint8_t *)mlir_aie_mem_alloc(xaie, buf1, output_length);
