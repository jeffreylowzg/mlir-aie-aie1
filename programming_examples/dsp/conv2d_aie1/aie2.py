...
def my_vector_scalar():
    ...
    input_total = input_size // 4

    output_size = Mout * Nout * CoutUp  #7744
    output_total = output_size // 4

    weight_size = F * F * CinUp * CoutUp
    weight_total = weight_size // 4
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            in_ty = T.memref(input_size, T.ui32())
            w_ty = T.memref(weight_size, T.i32())
            out_ty = T.memref(output_size, T.ui32())
            ...
            def sequence(A, B, C):
                ...
                npu_dma_memcpy_nd(metadata="weight", bd_id=1, mem=B, sizes=[1, 1, repeats, weight_total])
